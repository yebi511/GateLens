...
- “集群”控件平等列出所有已接入集群，切换后同步切换命名空间集合。
- 拓扑页面只展示当前集群，集群内按配置关系组织节点。跨命名空间边展示 `namespace/name` 和 ReferenceGrant/状态证据。
- 跨集群边界在当前集群对应配置对象上展示目标集群、目标服务或网关、传输方式与证据；远端未接入时显示“远端未接入”，不能继续展示虚构的 Route 或 Endpoint。
